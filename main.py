...
def insert_events(color):
    ...
    for i in range(0, max_rows):
        if is_all_day_event(i):
            ...
            print("inserting:" + subject[i] + ":" + 
                  convert_to_RFC_datetime(int(start_year[i]), int(start_month[i]), int(start_day[i]), adjust_time(int(fstart_time[i]), adjust_timezone) ,0) + "to" + 
                  convert_to_RFC_datetime(int(end_year[i]), int(end_month[i]), int(end_day[i]), adjust_time(int(fend_time[i]), adjust_timezone), 0) + 
                  "\nat" + location[i])
            service.events().insert(calendarId=id, body=event_request_body).execute()
